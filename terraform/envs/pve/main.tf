...
module "fleet" {
  ...
  # Old infra and old apps remain off during the migration soak; retain all VM and disk resources.
  on_boot         = !contains([201, 202, 203, 211, 212, 213], each.value.vmid)
  started         = !contains([201, 202, 203, 211, 212, 213], each.value.vmid)
  name            = each.key
