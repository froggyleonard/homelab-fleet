...
{{/*
FLEET PATCH (orchestrator task 026): pod placement.
Upstream 0.15.0 exposes no nodeSelector/affinity anywhere. The backend and
celery worker mount the same ReadWriteOnce attachments PVC, so on a
multi-worker cluster they must land on one node or the second pod fails with a
multi-attach error. Everything is pinned to one node until Longhorn RWX is
proven. Remove this patch when upstream ships scheduling knobs.
*/}}
{{- define "securo.placement" -}}
{{- with .Values.global.placement }}
{{- with .nodeSelector }}
nodeSelector:
  {{- toYaml . | nindent 2 }}
{{- end }}
